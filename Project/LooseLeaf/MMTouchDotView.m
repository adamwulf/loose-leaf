...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [touchGesture addTarget:self action:@selector(touchedWith:)];
...
-(void) touchedWith:(MMTouchDotGestureRecognizer*)gesture{
    NSMutableSet* seenKeys = [NSMutableSet set];
    for(UITouch* t in gesture.activeTouches){
    ...
    for(UIView* v in self.subviews){
        if(v.tag != NSUIntegerMax){
            if(![seenKeys containsObject:[NSNumber numberWithUnsignedInteger:v.tag]]){
