...
@implementation JotViewStateProxy (Trash)
...
+(void)load{
    @autoreleasepool {
        NSError *error = nil;
        [JotViewStateProxy jr_swizzleMethod:@selector(hasEditsToSave)
                                 withMethod:@selector(swizzle_hasEditsToSave)
                                      error:&error];
        [JotViewStateProxy jr_swizzleMethod:@selector(dealloc)
